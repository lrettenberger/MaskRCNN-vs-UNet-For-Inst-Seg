...
config_files = '/home/ws/kg2371/projects/sem-segmentation/DLIP/experiments/configurations/inst_seg_high_mag_maskedrcnn.yaml'
result_dir = './results_onnx'
...
w = torch.load('/home/ws/kg2371/projects/sem-segmentation/dnn_weights_masked_rcnn.ckpt')
model.load_state_dict(w['state_dict'])

x = torch.randn(1, 1, 1024, 1638, requires_grad=True)
...
torch.onnx.export(model, 
                  ...
                  "model_high_mag_maskedrcnn.onnx",   # where to save the model (can be a file or file-like object)
                  export_params=True,        # store the trained parameter weights inside the model file
                  opset_version=11,          # the ONNX version to export the model to
                  do_constant_folding=True,  # whether to execute constant folding for optimization
                  input_names = ['input'],   # the model's input names
                  output_names = ['output'], # the model's output names
                  dynamic_axes={'input' : {0 : 'batch_size', 2: 'x', 3: 'y'},    # variable length axes
                                'output' : {0 : 'batch_size'}})
...
np.testing.assert_allclose(to_numpy(torch_out[0]['boxes']), ort_outs[0], rtol=1e-03, atol=1e-05)
np.testing.assert_allclose(to_numpy(torch_out[0]['labels']), ort_outs[1], rtol=1e-03, atol=1e-05)
np.testing.assert_allclose(to_numpy(torch_out[0]['scores']), ort_outs[2], rtol=1e-03, atol=1e-05)
np.testing.assert_allclose(to_numpy(torch_out[0]['masks']), ort_outs[3], rtol=1e-03, atol=1e-05)
