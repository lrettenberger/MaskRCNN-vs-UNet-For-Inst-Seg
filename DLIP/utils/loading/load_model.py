...
def load_model(model_params: dict, checkpoint_path_str = None):
    ...
    if "optimizer" in optimizer_config:
        ...
        optimizer_params_refiltered = {}
        optimizer_params_split = split_parameters(dict_to_config(optimizer_params))
        # first add all general params
        for key in optimizer_params_split['other'].keys():
            optimizer_params_refiltered[key] = optimizer_params_split['other'][key]
        if optimizer_config['type'].lower() in optimizer_params_split.keys():
            # add specific params
            for key in optimizer_params_split[optimizer_config['type'].lower()].keys():
                optimizer_params_refiltered[key] = optimizer_params_split[optimizer_config['type'].lower()][key]
        optimizer_params = optimizer_params_refiltered
        optimizer_params['params'] = model.parameters()
        optim_instance = optimizer(**optimizer_params)
        lrs = None
        lrs_instance = None
