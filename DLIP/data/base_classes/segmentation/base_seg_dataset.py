...
class BaseSegmentationDataset(BaseDataset):
    def __init__(
        ...
        samples_data_format="tiff",
        labels_data_format="tiff",
        transforms = None,
        empty_dataset=False,
    ...
    def __getitem__(self, idx):
        ...
        if self.return_weight_maps:
            weight_map_path = sample_path.replace(self.samples_dir,'labels_not_confident')
            weight_map = tifffile.imread(weight_map_path) if self.samples_data_format=="tiff" else cv2.imread(weight_map_path,-1)
        else:
            # just some dummy weights that program does not crash
            weight_map = sample_img
            label_path = os.path.join(self.labels, f"{self.indices[idx]}.{self.labels_data_format}")
            distance_maps = False
            if 'distance_maps' in label_path:
                distance_maps = True
                weight_map = tifffile.imread(label_path) if self.labels_data_format=="tiff" else cv2.imread(label_path,-1)
        ...
        if self.labels_available:
            # load label map
            if isinstance(self.labels,str):
                label_path = os.path.join(self.labels, f"{self.indices[idx]}.{self.labels_data_format}").replace('distance_maps','labels')
                label_img = tifffile.imread(label_path) if self.labels_data_format=="tiff" else cv2.imread(label_path,-1)
                label_one_hot = label_img
                label_one_hot = label_img*1.
        ...
        if isinstance(self.labels,str):
            for transform in self.transforms:
                lbl = torch.zeros((256,512,512))
                for label_index in range(len(label_one_hot)):
                    random.seed(self.internal_seed)
                    im, lbl_transform, trafo = transform(sample_img, label_one_hot[label_index])
                    lbl[label_index] = lbl_transform
        ...
        if not self.return_trafos and self.labels_available and not self.return_weight_maps:
            if distance_maps:
                return sample_img_lst, weight_map_lst.unsqueeze(2), label_lst
            else:
