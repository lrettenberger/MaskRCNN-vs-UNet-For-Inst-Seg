...
def visualize_instances_map(
    ...
    """
    overlay = np.copy((input_image).astype(np.int16))
    ...
    inst_rng_colors = inst_rng_colors.astype(np.int16)

    for inst_idx, inst_id in enumerate(inst_list):
        inst_map_mask = np.array(inst_map == inst_id, np.int16)  # get single object
