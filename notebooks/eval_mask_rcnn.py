...
def post_process(pred,min_activation,lower_threshold,high_threshold,overlapping_threshold):
    C,H,W = pred['masks'][0].shape
...
model = load_model(parameters_splitted["model"],
                 checkpoint_path_str='/home/ws/kg2371/projects/sem-segmentation/results/first-shot/GenericSegmentationDataModule/MaskedRCNN/0923/dnn_weights.ckpt'
)
...
x = torch.zeros((0,1,1200,1920))
y_true = torch.zeros((0,1200,1920,1))
y_true_not_confident = torch.zeros((0,1200,1920,1))
y_pred = []
...
f = open(f"/home/ws/kg2371/projects/sem-segmentation/hypersearch/hyperparams_big_imgs_fine.txt", "a")
f.write('overlapping_threshold,min_activation,threshold,threshold_not_conf,aji\n')
for overlapping_threshold in tqdm(np.arange(0.35,0.45,0.02)):
    for min_activation in tqdm(np.arange(0.45,0.55,0.02)):
        for threshold_conf in np.arange(0.75,0.85,0.04):
            ajis_confident = []
            for k in range(len(x)):
                masks_summed_confident = post_process(
                    pred=y_pred[k],
                    min_activation=min_activation,
                    lower_threshold=threshold_conf,
                    high_threshold=1.0,
                ...
                try:
                    ...
                    print(e)
            f.write(f'{overlapping_threshold},{min_activation},{threshold_conf},{np.mean(np.array(ajis_confident))}\n')
            f.flush()
f.close()